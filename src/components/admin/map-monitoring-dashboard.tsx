...
type GeoFeature = { type: "Feature"; properties: Record<string, string>; geometry: MapGeometry };
...
function matchRegion(feature: GeoFeature, regions: MockRegion[]): MockRegion | undefined {
  ...
  // 이 GeoJSON은 2013 KOSTAT 코드 체계를 사용한다. 현재 행정코드와 숫자가 겹치지만
  // 의미가 다른 지역(예: GeoJSON 31=경기, 현재 31=울산)이 있으므로 이름으로만 매칭한다.
  return regions.find((item) => item.name === shortName);
}
...
export function MapMonitoringDashboard() {
  ...
  const projection = useMemo(() => projectMapFeatures(geoFeatures, geoCode), [geoFeatures]);
  const projectedFeatures = projection.paths;
  const projectedLabels = projection.labels;
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent className="p-0">
            <div className={`relative overflow-hidden bg-gradient-to-br from-slate-50 via-sky-50/50 to-emerald-50/60 ${level === "sido" ? "min-h-[680px]" : "min-h-[600px]"}`}>
              {mapLoading ? <div className={`flex items-center justify-center text-sm text-muted-foreground ${level === "sido" ? "h-[680px]" : "h-[600px]"}`}>실제 행정구역 경계를 불러오는 중입니다.</div> : mapError ? <div className={`flex items-center justify-center px-6 text-center text-sm text-destructive ${level === "sido" ? "h-[680px]" : "h-[600px]"}`}>{mapError}</div> : <svg viewBox="0 0 500 500" className={`mx-auto w-full ${level === "sido" ? "h-[680px] max-w-[860px]" : "h-[600px] max-w-[960px]"}`} role="img" aria-label="대한민국 3D 행정구역 지도"><defs><filter id="regionShadow" x="-20%" y="-20%" width="140%" height="150%"><feDropShadow dx="0" dy="5" stdDeviation="4" floodOpacity=".16" /></filter></defs><g filter="url(#regionShadow)">{geoFeatures.map((feature) => { const code = geoCode(feature); const name = geoName(feature); return <path key={`depth-${code}-${name}`} d={projectedFeatures.get(code)} fill="#526174" stroke="#334155" strokeWidth="1" opacity=".42" transform="translate(0 5)" />; })}{geoFeatures.map((feature) => { const code = geoCode(feature); const name = geoName(feature); const region = matchRegion(feature, regions); const active = region?.code === selected?.code; return <g key={`${code}-${name}`} transform={active ? "translate(0 -3)" : undefined} className="transition-transform duration-200"><path d={projectedFeatures.get(code)} fill={active ? "#2f6b55" : "#94a3b8"} stroke={active ? "#ffffff" : "rgba(255,255,255,.82)"} strokeWidth={active ? 3 : 1} className="cursor-pointer transition-all duration-200 hover:fill-slate-500" onClick={() => region && drillDown(region)}><title>{name || region?.name || code}</title></path></g>; })}{geoFeatures.map((feature) => { const code = geoCode(feature); const placement = projectedLabels.get(code); const rawName = geoName(feature); const region = matchRegion(feature, regions); const label = level === "sido" ? (region?.name ?? SIDO_SHORT_NAME[rawName] ?? rawName) : rawName; if (!placement || !label) return null; const levelMax = level === "sigungu" ? 9 : 11; const fontSize = Math.min(levelMax, placement.width / Math.max(label.length * .9, 1), placement.height * .34); const minimumWidth = level === "sido" ? 34 : 24; if (placement.width < minimumWidth || fontSize < 5) return null; return <text key={`label-${code}-${label}`} x={placement.x} y={placement.y - (selected?.code === region?.code ? 3 : 0)} textAnchor="middle" dominantBaseline="middle" fontSize={fontSize} className="pointer-events-none select-none fill-white font-bold [paint-order:stroke] stroke-black/20 stroke-[1.5px]">{label}</text>; })}</g></svg>}
